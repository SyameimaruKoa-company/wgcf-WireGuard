...
ワークフローを実行した際に入力した識別名 (`${{ github.event.inputs.terminal_name }}`) に基づいて、以下の2種類の WireGuard プロファイルが生成され、リリースにドラフトとして添付されます。ドラフトは非公開です。

## 生成ファイルリスト

* `WARP-CloudFlare-[端末名].conf`
    *   `1.1.1.1` `1.0.0.1` `2606:4700:4700::1111` `2606:4700:4700::1001`
    * これは Cloudflare の DNS サーバー を使用する WireGuard プロファイルです。
...
※ AdGuard と Google DNS は Cloudflare DNS プロファイルを基に、DNS 設定部分のみを置換しています。
